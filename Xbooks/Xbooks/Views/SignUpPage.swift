//
//  SignUpPage.swift
//  Xbooks
//
//  Created by Yusuf Özgül on 7.11.2020.
//

import UIKit

class SignUpPage: UIViewController {
    @IBOutlet weak var nameTextfiled: UITextField!
    @IBOutlet weak var surnameTextfiled: UITextField!
    @IBOutlet weak var mailTextfiled: UITextField!
    @IBOutlet weak var passTextfiled: UITextField!
    override func viewDidLoad() {
        super.viewDidLoad()

        // Do any additional setup after loading the view.
    }
    
    @IBAction func signup(_ sender: Any) {
        let body = SignupData(personName: nameTextfiled.text!, personLastName: surnameTextfiled.text!, personEmail: mailTextfiled.text!, password: passTextfiled.text!)
        let req = SignupRequest(body: body)
        ApiService<GenericResponse<LoginResponse>>().getData(request: req) { (result) in
            switch result {
            case .success(let data):
                if !data.error {
                    if let token = data.data?.token {
                        DispatchQueue.main.async {
                            UserDefaults.standard.setValue(token, forKey: "TOKEN")
                            let mainTabbar = MainTabbar()
                            mainTabbar.modalPresentationStyle = .fullScreen
                            self.present(mainTabbar, animated: true)
                        }
                    }
                } else {
                    print(data.message)
                }
            case .failure(let error):
                print(error.localizedDescription)
            }
        }
    }

}

struct SignupData: Codable {
    let personName: String
    let personLastName: String
    let personEmail: String
    let password: String
}

struct SignupRequest: ApiRequestProtocol {
    var method: HttpMethod
    var url: String
    var body: Encodable?
        
    init(body: Encodable) {
        method = .POST
        url = Constant.baseUrl + "signup"
        self.body = body
    }
}
